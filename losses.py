...
# Code heavily inspired in: https://github.com/LIVIAETS/boundary-loss
...
# CrossEntropyLoss = CrossEntropyLoss() # Pytorch
# diceLoss =GeneralizedDiceLoss(to_onehot_y=True, softmax=True) # MONAI
# focalLoss = FocalLoss(gamma=2) # MONAI
#
# boundary_loss = BoundaryLoss(idc=[1])
# HL = HausdorffLoss(idc=[1])
...
# ddd = BalancedPatchGenerator(path,(90, 90, 32),positive_prob=0.7,shuffle_images=True, mode='train',transform=None)
# loader = DataLoader(ddd, batch_size=1, shuffle=True)
...
#     y_cel = y.type(torch.LongTensor)
#     CEL = CrossEntropyLoss(out_probs,y_cel.squeeze(dim=0))
#     print('Cross entropy:',CEL.item())
#
#     # Generalized Dice Loss (MONAI)
#     dice = diceLoss(out,y)
#     print('GDL:',dice.item())
#
#
#     # Hausdorff loss
#     hausdorff = HL(out_probs,y.repeat(1,2,1,1,1))
#     print('Hausdorff Loss',hausdorff.item())
#
#
#     # Boundary Loss
#     bl_loss = boundary_loss(out_probs, y)
#     print('Boundary_loss:', bl_loss.item())
#
#
#     plt.imshow(y[0,0,:,:,15],cmap='gray')
#     plt.show()
#
#     plt.imshow(dist[0,0,:,:,15],cmap='gray')
#     plt.show()
#
#     plt.imshow(dist[0,1,:,:,15],cmap='gray')
#     plt.show()
#
