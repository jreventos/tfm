...
# smoothing = GaussianSmoothing(2, 5, 1)
# input = torch.rand(1, 2, 100, 100,60)
# input = F.pad(input, (2, 2, 2, 2, 2, 2), mode='replicate')
# output = smoothing(input)
# print(output.detach().cpu().numpy()[0,0,:,:,30])
#
# plt.figure("check", (18, 6))
# plt.subplot(1, 2, 1)
# plt.imshow(input.detach().cpu().numpy()[0,0,:,:,30])
# plt.subplot(1, 2, 2)
# plt.imshow(output.detach().cpu().numpy()[0,0,:,:,30])
# plt.show()
#
...
def remove_small_objects(img):
    ...
    isolated_numpy = torch.tensor(np.expand_dims(new_img, axis=(0, 1)))
